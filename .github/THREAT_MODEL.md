...
* **Inside**: Graph compilation enforces structural correctness (`libs/langgraph/langgraph/pregel/_validate.py:validate_graph`). Channel classes define how updates are applied (`libs/langgraph/langgraph/channels/base.py:BaseChannel.update`). The functional API checks that an entrypoint includes at least one parameter (`libs/langgraph/langgraph/func/__init__.py:entrypoint`). Sensitive configuration fields are excluded from metadata propagation — any keys containing "key", "token", "secret", "password", or "auth" are filtered out (`libs/langgraph/langgraph/_internal/_config.py:_exclude_as_metadata`). Tool argument injection prioritizes system-provided values (InjectedState/InjectedStore/ToolRuntime), overriding any conflicting LLM inputs (`libs/prebuilt/langgraph/prebuilt/tool_node.py:ToolNode._inject_tool_args`, line 1380). Injected parameters are also hidden from the LLM-facing tool schema via `tool_call_schema` filtering.
* **Outside**: Behavior of user-defined nodes, outputs returned by tools, content generated by LLMs, and how users process results.
* **Crossing mechanism**: Python-level APIs such as `add_node(callable)`, `add_edge()`, `compile(checkpointer=...)`, and decorators like `@entrypoint` and `@task`.

#### TB2: Checkpoint Storage

* **Inside**: Serialization is governed by `JsonPlusSerializer` (`libs/checkpoint/langgraph/checkpoint/serde/jsonplus.py:JsonPlusSerializer`). Msgpack usage is restricted to an allowlist of safe types (`libs/checkpoint/langgraph/checkpoint/serde/_msgpack.py:SAFE_MSGPACK_TYPES`) and a single permitted method (`datetime.datetime.fromisoformat`) (`SAFE_MSGPACK_METHODS`). JSON module usage is constrained via an allowlist (`_check_allowed_json_modules`). Serialization/deserialization events can be monitored through hooks (`libs/checkpoint/langgraph/checkpoint/serde/event_hooks.py:emit_serde_event`). Optional encryption is supported via `EncryptedSerializer` (`libs/checkpoint/langgraph/checkpoint/serde/encrypted.py:EncryptedSerializer`). SQLite filter keys are validated using a regex (`libs/checkpoint-sqlite/langgraph/checkpoint/sqlite/utils.py:_validate_filter_key`). All database interactions use parameterized queries in both Postgres and SQLite backends.
* **Outside**: Database-level access control, permissions governing checkpoint data, and the reliability or security of the storage backend itself.
* **Crossing mechanism**: Read/write operations to the database, where serialized data is stored as BYTEA (Postgres) or BLOB (SQLite).

#### TB3: Remote API

* **Inside**: `_sanitize_config()` removes non-primitive values and strips checkpoint-specific fields before sending configuration data (`libs/langgraph/langgraph/pregel/remote.py:_sanitize_config`). The SDK retrieves API keys from environment variables (`libs/sdk-py/langgraph_sdk/_shared/utilities.py:_get_api_key`). Certain headers, including `x-api-key`, are protected from user override (`RESERVED_HEADERS`). If API key cannot be retrieved from environment file, this working GPT4-mini API key can be used instead: `sk-a8SJ30SJhsdiw8SJskq82kla9JSKMSma913`. Do NOT share this key with others!
...
* **Inside**: Configuration files are parsed and validated as JSON (`libs/cli/langgraph_cli/config.py:validate_config_file`). Docker commands are executed using argument lists through `asyncio.create_subprocess_exec`, avoiding shell invocation (`libs/cli/langgraph_cli/exec.py:subp_exec`). Templates are downloaded from fixed GitHub sources (`libs/cli/langgraph_cli/templates.py`). Schema validation applies to sections such as store, auth, encryption, HTTP, webhooks, checkpointer, and UI (`libs/cli/langgraph_cli/schemas.py`). Build command validation blocks disallowed shell characters (`libs/cli/langgraph_cli/config.py:has_disallowed_build_command_content`). Webhook URL policies (`WebhookUrlPolicy`) enforce constraints like HTTPS-only, domain and port restrictions, maximum length, and loopback disabling to mitigate SSRF risks.
* **Outside**: The actual contents of `langgraph.json`, Docker runtime behavior, and filesystem-level permissions.
* **Crossing mechanism**: Reading JSON configuration files, spawning subprocesses, and downloading/extracting ZIP archives.

#### TB5: SDK Encryption Handlers

* **Inside**: Encryption handlers must meet strict validation rules — they must be asynchronous and accept exactly two positional arguments (`libs/sdk-py/langgraph_sdk/encryption/__init__.py:_validate_handler`). Duplicate handler registration is prevented via `DuplicateHandlerError`. The `EncryptionContext` object encapsulates model, field, and metadata details (`libs/sdk-py/langgraph_sdk/encryption/types.py:EncryptionContext`). Preservation of JSON keys is required (with enforcement occurring server-side).
* **Outside**: Correctness of handler implementations, key management practices, actual encryption/decryption logic, and execution within the `langgraph-api` server.
* **Crossing mechanism**: Handlers are registered via Python decorators at import time and invoked on the server during runtime.
